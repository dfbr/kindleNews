...
def load_config(root: Path, config_path: Path | None = None) -> AppConfig:
    ...
    selection_payload = dict(merged["selection"])
    selection_payload.pop("min_pages", None)
    selection_payload.pop("max_pages", None)
    selection_payload.pop("words_per_page", None)
    app.selection = SelectionConfig(**selection_payload)
    app.dedupe = DedupeConfig(**merged["dedupe"])
    ai_payload = dict(merged["ai"])
    ai_payload.pop("summary_prompt_file", None)
    app.ai = AIConfig(**ai_payload)
    app.ai.ranking_prompt_file = Path(ai_payload["ranking_prompt_file"])
