...
def test_run_clears_daily_cache_immediately_after_epub_build(monkeypatch, tmp_path: Path) -> None:
    root = tmp_path
    config_dir = root / "config"
    config_dir.mkdir(parents=True)
    (config_dir / "config.yaml").write_text(
...
""",
    ...
    (cache_dir / "daily-2026-04-16.json").write_text("[]", encoding="utf-8")
    ...
    def fake_send_epub(*args, **kwargs) -> None:
        assert list(cache_dir.glob("daily-*.json")) == []

    monkeypatch.setattr("kindle_news.pipeline.send_epub", fake_send_epub)
